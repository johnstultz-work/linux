...
static inline struct task_struct *__blocked_on_owner(struct blocked_on_lock *bo)
{
	...
	case BO_T_RT_MUTEX:
		/* XXX needs CONFIG_RT_MUTEXES */
		return rt_mutex_owner(bo->lock);
	default:
		BUG();
	}
}
...
lock_blocked_on_lock(struct blocked_on_lock *bo)
{
	...
	else if (bo->type == BO_T_RT_MUTEX)
		raw_spin_lock(&((struct rt_mutex_base *)bo->lock)->wait_lock);
	else
		BUG();
}
...
unlock_blocked_on_lock(struct blocked_on_lock *bo)
{
	if (bo->type == BO_T_MUTEX)
		...
		raw_spin_unlock(&((struct rt_mutex_base *)bo->lock)->wait_lock);
	else
		BUG();
}
